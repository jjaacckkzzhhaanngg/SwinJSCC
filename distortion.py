...
class MS_SSIM(torch.jit.ScriptModule):
    ...
    def forward(self, X, Y):
        ...
        return 1 - ms_ssim(X, Y, window=window, data_range=self.data_range, weights=weights,
...
class Distortion(torch.nn.Module):
    def __init__(self, config):
        """
        初始化失真计算模块
        :param config: SimpleNamespace 类型的配置对象，包含：
            - distortion_metric: 失真度量类型 ('MSE'/'SSIM'/'MS-SSIM')
            - trainset: 数据集名称 (如 'CIFAR10')
            - logger: 日志记录器 (可选)
        """
        ...
        if config.distortion_metric == 'MSE':
            ...
            window_size = getattr(config, 'ms_ssim_window_size', 11)
            levels = getattr(config, 'ms_ssim_levels', 3)
            self.dist = MS_SSIM(
            ...
            raise ValueError(f"未知的失真度量类型: {config.distortion_metric}")
...
if __name__ == '__main__':
    # 测试代码 - 创建 SimpleNamespace 配置
    test_config = SimpleNamespace(
        distortion_metric='MS-SSIM',
        trainset='tiny-imagenet-200',
        logger=None
    )
    
    # 动态选择设备（优先CUDA，无则用CPU）
    ...
    print(f"MS-SSIM Loss: {loss}")
    
    # 测试 Distortion 类
    distortion = Distortion(test_config).to(device)
    dist_loss = distortion(rand_im1, rand_im2)
    print(f"Distortion Loss: {dist_loss}")